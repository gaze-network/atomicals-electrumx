...
class GazeNetworkReportClient:
    ...
    def submit_block_report(
        self, type: str, height: int, block_hash: bytes, event_hash: bytes, cumulative_event_hash: bytes
    ):
        ...
        if resp.status_code >= 400:
            self.logger.warning(
                f"submit_block_report: Failed to submit block report. request_body={json.dumps(data)} status_code={resp.status_code}, text={resp.text}"
            )
        else:
            self.logger.info(
                f"submit_block_report: Submitted block report to Gaze Network: type {type} height {height}, block_hash {block_hash_str}, event_hash {event_hash_str}, cumulative_event_hash {cumulative_event_hash_str}"
            )
    ...
    def submit_node_report(self, type: str):
        ...
        if resp.status_code >= 400:
            self.logger.warning(
                f"submit_node_report: Failed to submit node report. request_body={json.dumps(data)}, status_code={resp.status_code}, text={resp.text}"
            )
        else:
            self.logger.info(
                f'submit_node_report: Submitted node report to Gaze Network. name={data["name"]}, type={data["type"]}, websiteURL={data["websiteURL"]}, indexerAPIURL={data["indexerAPIURL"]}'
            )
