...
class HttpOPIHandler(object):
    def __init__(self, session_mgr: SessionManager, env: "Env", db: DB, bp: BlockProcessor):
    ...
    async def get_block_height(self, request: "Request") -> "Response":
        ...
        return format_response({"block_height": block_height})
    ...
    async def _get_populated_arc20_balances(self, address: str, atomical_id: "bytes | None", block_height: int):
        ...
        history_data = [
            ...
            if self.env.coin.ATOMICALS_ACTIVATION_HEIGHT <= x["height"] and x["height"] <= block_height
        ]
    ...
    async def get_arc20_balance(self, request: "Request") -> "Response":
        ...
        return format_response({"balances": populated_balances})
            ...
            if not address:
                if not pk_script:
                    return format_response(
                        None, 400, f'query index {idx}: either "pk_script" or "address" must be provided'
                    )
                address = get_address_from_output_script(bytes.fromhex(pk_script))
    ...
    async def get_arc20_activity(self, request: "Request") -> "Response":
        ...
        for tx in tx_hashes:
            ...
            txs.append({"tx_num": tx_num, "tx_hash": tx, "height": block_height})

        txs.sort(key=lambda x: x["tx_num"])
        tx_datas = await asyncio.gather(
            *[self.session_mgr.get_transaction_detail(tx["tx_hash"], block_height, tx["tx_num"]) for tx in txs]
        )
        result = []
        for tx_data in tx_datas:
        ...
        total = len(result)
        return format_response({"total": total, "block_height": block_height, "list": result[offset : offset + limit]})
